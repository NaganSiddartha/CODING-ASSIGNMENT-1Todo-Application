...
const hasCatogery = requestQuery => {
...
const convertIntotodoObject = dbObject => {
  return {
    id: dbObject.id,
    todo: dbObject.todo,
...
app.get('/todos/', async (request, response) => {
  ...
  switch (true) {
    ...
    case hasPriorityAndStatus(request.query):
      ...
      WHERE
        ...
        AND priority = '${priority}';`
          ...
          response.send(
            data.map(singleTodo => convertIntotodoObject(singleTodo)),
          )
        } else {
          response.status(400)
    ...
    case hasCatogeryAndStatus(request.query):
      ...
      break
    //3
    case hasCatogery(request.query):
      if (
        category === 'WORK' ||
        category === 'HOME' ||
        category === 'LEARNING'
      ...
      category = '${category}';`
        data = await database.all(getTodosQuery)
        response.send(data.map(singleTodo => convertIntotodoObject(singleTodo)))
      } else {
        response.status(400)
        response.send('Invalid Todo Category')
      }
...
app.get('/todos/:todoId/', async (request, response) => {
  ...
  response.send(convertIntotodoObject(data))
})
...
app.get('/agenda/', async (request, response) => {
  ...
  if (isMatch(date, 'yyyy-MM-dd')) {
    ...
    const data = await database.all(requestQuery)
